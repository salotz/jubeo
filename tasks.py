...
def repo_test(cx):
    ...
    with cx.prefix(" jubeo.dev"):
        cx.run("conda activate jubeo.dev && inv -l",
               pty=True)
...
def docs_build(cx):
    ...
    with cx.cd('sphinx'):
        ...
        cx.run("sphinx-apidoc -f --separate --private --ext-autodoc --module-first --maxdepth 1 -o api ../src/jubeo")
...
def lint(cx):
    ...
    cx.run("flake8 --output-file=metrics/lint/flake8.txt src/jubeo")
...
def complexity(cx):
    ...
    cx.run("lizard -o metrics/code_quality/lizard.csv src/jubeo")
    cx.run("lizard -o metrics/code_quality/lizard.html src/jubeo")

    # SNIPPET: annoyingly opens the browser

    # make a cute word cloud of the things used
    # cx.run("(cd metrics/code_quality; lizard -EWordCount src/jubeo > /dev/null)")
...
def version_which(cx):
    ...
    from jubeo import __version__ as proj_version
    print(proj_version)
...
def publish_test(cx):
    ...
    from jubeo import __version__ as version
...
#     cx.run(f"cp conda/conda-forge {CONDA_FORGE_RECIPE_PATH}/jubeo")
